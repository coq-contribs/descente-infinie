...
all: $(VOFILES) $(CMOFILES) $(CMXSFILES) src/di_plugin.cmxs\
  src/di_plugin.cma
spec: $(VIFILES)
...
install:
	...
	(cd src; for i in $(CMOFILES1); do \
	 install -d `dirname $(COQLIB)/user-contrib/DescenteInfinie/$$i`; \
	 install $$i $(COQLIB)/user-contrib/DescenteInfinie/$$i; \
	 done)
...
clean:
	...
	- rm -rf html
	- rm -f src/di_plugin.cmxs
	- rm -f src/di_plugin.cma
